    ...
    _init_set_value: function(a, b, c) {
        ...
        function f(a) {
            for (var b = g[a], c = 0; c < b.length; c++)
                if (b[c].checked) return b[c].value
        }
...
scheduler.attachEvent("onBeforeEventDelete", function(a) {
    ...
    if (a.toString().indexOf("#") != -1 || b.event_pid && b.event_pid != "0" && b.rec_type != "none") {
        ...
        var e = this._get_rec_markers(a),
            h;
        for (h in e)
            if (e.hasOwnProperty(h)) a = e[h].id, this.getEvent(a) &&
...
scheduler.transpose_day_week = function(a, b, c, d, f) {
    for (var e = (a.getDay() || (scheduler.config.start_on_monday ? 7 : 0)) - c, h = 0; h < b.length; h++)
        if (b[h] > e) return a.setDate(a.getDate() + b[h] * 1 - e - (d ? c : f));
    ...
    if (!this.date[b]) {
        ...
        if (c[0] == "day" || c[0] == "week") {
            ...
            this.date[f] = function(a, b) {
                ...
                if (h)
                    for (var d = 0; d < b; d++) scheduler.transpose_day_week(c, h, 0, e);
